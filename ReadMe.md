...
3. Then paste and run `server/supabase/quota-period-param.sql` once. This changes `increment_usage()` to take an explicit period start (computed server-side) instead of always assuming a calendar month inside the function.
...
6. Tiers and their monthly search caps live in `TIER_LIMITS` in `server/src/auth.js`: `free` (10), `plus` (12, $4.99/mo), `pro` (25, $9.99/mo), `max` (50, $19.99/mo). Nothing charges for these yet — that's wired up once subscription billing (e.g. RevenueCat + Play Billing) exists; for now a user's tier is set manually in the `profiles` table for testing. AI ranking (Qwen) is gated to any paid tier — free accounts see the local ranking heuristic and an upgrade prompt instead of calling `/api/recommendations` at all.

### Cost controls & profit margin

Each paid tier's quota is sized so that even a subscriber using their *entire* monthly allowance (worst case — no cache hit, all `MAX_CANDIDATES_TO_ENRICH` places enriched) keeps SerpApi cost under ~55% of what they paid, guaranteeing at least a 30-40% profit margin after Google Play's ~15% cut. This is real math, not a guess — see the comment on `TIER_LIMITS` in `server/src/auth.js`. Raising a tier's quota, lowering its price, or raising `MAX_CANDIDATES_TO_ENRICH` in `server/src/places.js` (currently 6, i.e. 7 SerpApi calls/search) all erode that guarantee and need re-checking against the same math before shipping.

Free tier has no natural cost check — anyone can create unlimited free accounts — so it gets a shared monthly call budget on top of its per-account quota (`server/src/costGuard.js`, tunable via `SERPAPI_FREE_MONTHLY_CALL_BUDGET`). Once hit, `/api/places` pauses live search for free accounts only (a 402 response, client falls back to sample data) rather than continuing to spend. The default (400 calls) is a modest slice of SerpApi's $25/mo Starter plan (1,000 calls/month, ~7 calls per uncached search) — check your actual SerpApi plan and adjust the env var before relying on it.

Paid tiers are **not** subject to that shared budget. A paying customer's request is bounded only by their own advertised monthly quota (enforced via `usage_periods`) and by SerpApi's real account capacity — never by a pool that other accounts (free or paid) can exhaust out from under them. If paid usage ever grows enough to matter, that shows up directly in your SerpApi/HF billing dashboards; raise the SerpApi plan or adjust `TIER_LIMITS` at that point rather than relying on a shared cap to protect a paying customer's own service.
